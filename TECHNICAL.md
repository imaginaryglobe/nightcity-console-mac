# CyberConsole: how it works and how it was built

This document explains the internals: how CyberConsole injects into Cyberpunk 2077 on macOS, how it
...
CyberConsole rebuilds the core of that capability from scratch on macOS: inject code into the running
...
and crash. CyberConsole logs each function's live signature so commands are built against the real shape.
...
- `tools/sign-notarize.sh` re-signs everything inside-out with a Developer ID and hardened runtime,
  notarizes the app and the dmg with `notarytool`, staples the tickets, and produces `dist/CyberConsole.dmg`
